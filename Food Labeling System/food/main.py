...
from food.bayesian_network import bayesianNetwork
from food.clustering import kMeansCluster
from food.nutriscore_classification import knn_model
from food.read_csv import readCSV
...
class Dialogo(tk.Frame):
    def __init__(self):
        ...
        self.carb = 0
        self.gras = 0
        self.food_df = readCSV('../data/food_dataset_final.csv', ',')  # lettura del dataset
        self.hypers_knn = dict(leaf_size=1, n_neighbors=7, p=1, weights='distance')

        tk.Frame.__init__(self)
        self.master.title("Food Labeling System Nustri-Score")  # Diamo il titolo alla finestra.
        self.master.minsize(400, 150)  # Dimensioni minime della finestra.
        self.master.iconphoto(False, tk.PhotoImage(file='../food/images/icon.png'))
        ...
        self.calorie = tk.Label(self, text="Calorie (per 100g):")  # Etichetta delle calorie
        self.calorie.grid(column=0, row=0, sticky=tk.W, padx=5, pady=5)
        ...
        self.carboidrati = tk.Label(self, text="Carboidrati (per 100g):")  # Etichetta dei carboidrati
        self.carboidrati.grid(column=0, row=1, sticky=tk.W, padx=5, pady=5)
        ...
        self.proteine = tk.Label(self, text="Proteine (per 100g):")  # Etichetta delle proteine
        self.proteine.grid(column=0, row=2, sticky=tk.W, padx=5, pady=5)
        ...
        self.grassi = tk.Label(self, text="Grassi (per 100g):")  # Etichetta dei grassi
        self.grassi.grid(column=0, row=3, sticky=tk.W, padx=5, pady=5)

        self.entrata4 = tk.Entry(self)  # Casella d'inserimento dei grassi
        self.entrata4.grid(column=1, row=3, sticky=tk.E, padx=5, pady=5)

        self.start = tk.Button(self, text="Start", command=self.calcola)  # Bottone "Start"
        self.start.grid(column=1, row=4, sticky=tk.E, padx=5, pady=5)
        self.reset = tk.Button(self, text="Reset", command=self.reset)  # Bottone "Reset"
        self.reset.grid(column=0, row=4, sticky=tk.W, padx=5, pady=5)

    # Raccogliamo l'input e calcoliamo
    def calcola(self):

        newWindow = tk.Toplevel(self)
        self.etichetta5 = tk.Label(newWindow, text="NUTRISCORE (A-E)")  # Etichetta del nutriscore
        self.etichetta5.grid(column=0, row=1, sticky=tk.W, padx=5, pady=5)

        self.risultato_knn = tk.Text(newWindow, height=1, width=50)  # Testo che mostra il risultato del knn.
        self.risultato_knn.grid(column=0, row=2, sticky=tk.E, padx=5, pady=5)

        self.etichetta5 = tk.Label(newWindow, text="È SALUTARE?")  # Etichetta ALIMENTO SALUTARE-NON SALUTARE
        self.etichetta5.grid(column=0, row=3, sticky=tk.W, padx=5, pady=5)

        self.risultato_bayes = tk.Text(newWindow, height=1, width=50)  # Testo che mostra il risultato del knn.
        self.risultato_bayes.grid(column=0, row=4, sticky=tk.E, padx=5, pady=5)

        self.etichetta5 = tk.Label(newWindow, text="ALIMENTI SIMILI")  # Etichetta alimenti simili
        self.etichetta5.grid(column=0, row=5, sticky=tk.W, padx=5, pady=5)

        self.risultato_kmeans = tk.Text(newWindow, height=20, width=50)  # Testo che mostra il risultato del k-means.
        self.risultato_kmeans.grid(column=0, row=6, sticky=tk.E, padx=5, pady=5)
    ...
    def checkString(self, feature, numberControl):
        ...
        if feature == "grassi" or feature == "carboidrati" or feature == "proteine":
            try:
                float(numberControl)
                if float(numberControl) > 100 or float(
                        numberControl) < 0:  # controllo se i valori in input di grassi proteine e carbo siano compresi tra 0 e 100
                    self.reset
                    errMex = 1
                    ret = 1
            ...
            try:
                float(numberControl)
                if (float(numberControl) > 1000 or float(
                        numberControl) < 0):  # controllo se il valore in input di calorie sia compreso tra 0 e 1000
                    self.reset
                    errMex = 1
                    ret = 1
            except ValueError:
